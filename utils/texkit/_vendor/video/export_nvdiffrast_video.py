...
class VideoExporter:
    ...
    def export_condition(
        ...
        scale=0.85, fov_deg=49.1, perspective=False, orbit=True, c2ws=None,
        normal_map_strength=1.0,  # 添加法线贴图强度参数
        ...
        texture_mesh = Texture.from_trimesh(mesh)
        ...
        # initialize renderer
        if c2ws is not None:
            assert c2ws.shape[0] == n_views, f'Value Error: c2ws.shape[0]={c2ws.shape[0]}, n_views={n_views}'
        elif orbit:
        ...
        normal_bump = out['world_normal_bump'].mul(0.5).add(0.5)
        if background is not None:
            normal_bump = normal_bump * alpha + background * (1.0 - alpha)
        
        # 获取 normal map texture
        normal_map_texture = out.get('normal_map_texture', None)
        ...
        if not return_image:
            results = {
                ...
                'normal_bump': normal_bump.detach().cpu().numpy(),
            }
            if return_mesh:
                results.update({'mesh': mesh})
            if return_camera:
                results.update({'c2ws': c2ws, 'intrinsics': intrinsics, 'perspective': perspective})
            # 添加 normal map texture 到返回结果
            if normal_map_texture is not None:
                results.update({'normal_map_texture': normal_map_texture})
            return results
        ...
        normal_bump_im = normal_bump.clamp(0.0, 1.0).mul(255.0).detach().cpu().numpy().astype(np.uint8)
        normal_bump_im = Image.fromarray(normal_bump_im.reshape(n_rows, n_cols, H, W, 3).transpose(0, 2, 1, 3, 4).reshape(n_rows * H, n_cols * W, 3), mode='RGB')
        
        results = {
            'alpha': alpha_im,
            'ccm': ccm_im,
            'normal': normal_im,
            'normal_bump': normal_bump_im,
        }
        if return_mesh:
            results.update({'mesh': mesh})
        if return_camera:
