        ...
        if self.backend in ['scipy']:
            ...
            if batch_size is None:
                ...
                for idx in tqdm(range(0, M, batch_size), 'knn'):
                    # FIXME:
                    # score[idx:idx+batch_size, :], index[idx:idx+batch_size, :] = self.kdtree.query(dst[idx:idx+batch_size, :], k=k, workers=8, distance_upper_bound=2 * math.sqrt(3))
                    end_idx = min(idx + batch_size, M)
                    batch_score, batch_index = self.kdtree.query(dst[idx:end_idx, :], k=k, distance_upper_bound=2 * math.sqrt(3))
                    # Ensure 2D shape for consistent indexing
                    batch_score = batch_score.reshape(-1, k) if batch_score.ndim == 1 else batch_score
                    batch_index = batch_index.reshape(-1, k) if batch_index.ndim == 1 else batch_index
                    score[idx:end_idx, :] = batch_score
                    index[idx:end_idx, :] = batch_index
            score = torch.as_tensor(score, dtype=torch.float32, device=device)
            index = torch.as_tensor(index, dtype=torch.int64, device=device)
            dt2 = perf_counter() - t
...
def knn(src:torch.Tensor, dst:torch.Tensor, k:int=1, backend='cupy', batch_size=None, return_dt=False) -> Tuple[torch.Tensor, torch.Tensor]:
    '''
    src: [N, C]
    dst: [M, C]
    backend: scipy, cupy, faiss, pytorch
