...
class NVDiffRendererBase(nn.Module):
    ...
    def simple_rendering(
        ...
        map_normal:Optional[Tensor]=None,  # 法线贴图参数
        normal_map_strength:float=1.0,    # 法线贴图强度控制参数
        background=None,
        ...
        **kwargs,
        ...
        map_normal: [H, W, 3] or [M, H, W, 3], normal map in tangent space
        normal_map_strength: float, strength of normal map effect
        render_world_normal: bool, render basic world normal from vertices
        render_world_normal_bump: bool, render world normal with bump map effect
        c2ws: [M, 4, 4]
        ...
        if render_world_normal:
            ...
            # 渲染带 bump map 效果的 world normal
            # 检查是否有法线贴图和必要的UV信息
            has_normal_map = (map_normal is not None and 
                            hasattr(mesh, '_v_tex') and mesh._v_tex is not None and
                            hasattr(mesh, '_t_tex_idx') and mesh._t_tex_idx is not None)
            ...
            if has_normal_map:
                ...
                # 没有法线贴图时回退到普通法线
                print("[NVDiffRendererBase] No normal map found for world_normal_bump, using vertex normals.")
                world_normal_bump, _ = dr.interpolate(mesh.v_nrm.contiguous(), rast, t_pos_idx)
                world_normal_bump = F.normalize(world_normal_bump, dim=-1)
            ...
            # 添加原始 normal map texture 到返回结果
            out.update({"normal_map_texture": map_normal})
        else:
            # 没有法线贴图时返回 None
            out.update({"normal_map_texture": None})
