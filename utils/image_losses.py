...
def compute_masked_mse_loss(pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor, weighting: torch.Tensor, view_weighting: torch.Tensor = None) -> torch.Tensor:
    """
    Compute weighted MSE loss with mask.
    
    Args:
        pred: Model prediction tensor
        target: Target tensor
        mask: Mask tensor (1 for valid pixels, 0 for invalid)
        weighting: Loss weighting tensor
    
    Returns:
        Computed MSE loss normalized by valid pixels
    """
    diff = (pred * mask - target * mask).float()
    if view_weighting is not None:
        diff = diff * view_weighting[:, None, None, None].float()
    return torch.sum(weighting.float() * diff ** 2) / mask.sum()


def compute_normalized_mse_loss(pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor, weighting: torch.Tensor) -> torch.Tensor:
    """
    Compute normalized MSE loss with mask.
    ...
    Returns:
        Normalized MSE loss
    """
    # Normalize pred and target to unit vectors
    pred_norm = F.normalize(pred, p=2, dim=-1)
    target_norm = F.normalize(target, p=2, dim=-1)
    
    diff = (pred_norm * mask - target_norm * mask).float()
    return torch.sum(weighting.float() * diff ** 2) / mask.sum()
