...
def get_class_by_name(name):
    ...
    package = __name__.rsplit(".", 1)[0]  # "lvsm"
    try:
...
def compute_rays(fxfycxcy, c2w, h, w):
    ...
    x = ((idx_x + 0.5) / w - 0.5) / (fxfycxcy[:, 0:1])     # [b*v, h*w]
    y = ((idx_y + 0.5) / h - 0.5) / (fxfycxcy[:, 1:2])     # [b*v, h*w]
    z = torch.zeros_like(x) 
    
    ray_o = torch.stack([x, y, z], dim=1)                 # [b*v, h*w]
    ones = torch.ones((b*v, 1, h*w), dtype=ray_o.dtype, device=ray_o.device)
    ray_o_h = torch.cat([ray_o, ones], dim=1)
    ray_o_h = torch.bmm(c2w, ray_o_h)
    ray_o = ray_o_h[:, :3]


    ray_d = c2w[:, :3, 2]   # [B, 3]
    ray_d = ray_d / ray_d.norm(dim=-1, keepdim=True)
    ray_d = ray_d[:, :, None].expand(b * v, 3, h*w)
...
class LaCTLVSM(nn.Module):
    def __init__(self, patch_size, dim, layers, block_config):
        ...
        self.pose_keys = ["ray_o", "ray_d", "o_cross_d", "normalized_normal", "normalized_ccm"] 
        ...
        with torch.autocast(device_type="cuda", enabled=False), torch.no_grad():
            ...
            for data_dict in [input_data_dict, target_data_dict]:
                ...
                if "image" in data_dict:
                    ...
                    data_dict["normalized_normal"] = data_dict["normal"] * 2.0 - 1.0
                    data_dict["normalized_ccm"] = data_dict["ccm"] * 2.0 - 1.0
