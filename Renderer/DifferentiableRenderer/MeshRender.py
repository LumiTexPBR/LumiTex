...
class MeshRender:
        ...
        if mode == RenderMode.ALPHA:
            ...
            if not hasattr(self, 'tex') or self.tex is None:
                ...
                if self.vtx_uv is not None and self.uv_idx is not None:
                    ...
                    uv_coords = uv_coords[0, ...]  # [H, W, 2]
                    
                    # 使用PyTorch的grid_sample进行纹理采样
    ...
    # 在MeshRender类内部添加纹理采样辅助方法（第1456行之后，注意缩进）
    def _sample_texture_grid(self, uv_coords: torch.Tensor) -> torch.Tensor:
        """
        使用PyTorch的grid_sample从纹理中采样颜色值
        
        Args:
            uv_coords: UV坐标张量 [H, W, 2]，值在[0, 1]范围内
            
        Returns:
            采样的颜色值 [H, W, 3]
        """
        # 将纹理从[H, W, C]转换为[1, C, H, W]格式用于grid_sample
        tex = self.tex.permute(2, 0, 1).unsqueeze(0)  # [1, 3, H, W]
        
        # 将UV坐标转换为grid_sample需要的格式
        # grid_sample期望坐标在[-1, 1]范围，而UV在[0, 1]范围
        grid = uv_coords * 2.0 - 1.0  # [H, W, 2] 转换到[-1, 1]
        
        # grid_sample需要[N, H, W, 2]格式的grid
        ...
        # 转换回[H, W, 3]格式
        result = sampled.squeeze(0).permute(1, 2, 0)  # [H, W, 3]
        
        return result

    def render_albedo(self, elev, azim, camera_distance=None, center=None, resolution=None,
                     bg_color=[1, 1, 1], return_type="th"):
        """
        渲染mesh的albedo（反照率）纹理颜色
        
        Args:
            elev: 相机仰角（度）
            azim: 相机方位角（度） 
            camera_distance: 相机距离，None时使用默认值
            center: 相机焦点中心，None时使用原点
            resolution: 渲染分辨率，None时使用默认值
            bg_color: 背景颜色 [R, G, B]
            return_type: 返回类型 ("th"=tensor, "np"=numpy, "pl"=PIL)
            
        Returns:
            渲染的albedo图像，格式由return_type决定
        """
        config = RenderConfig(elev, azim, camera_distance, center, resolution, bg_color, return_type)
