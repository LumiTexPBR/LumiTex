    ...
    float3 gb_kd, float3 gb_ks, float pdfSum, float weight, float3 &diff, float3 &spec, float3 diff_grad, float3 spec_grad)
{
    ...
    float3 _diff = make_float3(0.0), _spec = make_float3(0.0);
    ...
    // diff = 1 * float3{1.0,1.0,1.0} * 1 * 0.5 * weight * 1;
    // spec = 0.2 * float3{1.0,1.0,1.0} * 1 * 0.5 * weight * 1;
